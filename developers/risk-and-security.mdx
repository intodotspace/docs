...
<Steps>
  ...
  <Step title="Switch to call auction mode" titleSize="h3"> </Step>
...
**Insurance Fund:**
    ...
    - Target reserve: 2% of platform TVL
The insurance fund protects the platform from user accounts going negative due to rapid price movements or network congestion preventing timely liquidation.
...
If $ \frac{|\text{Actual Fill Price} - \text{Expected Price}|}{ \text{Expected Price}} > \text{Max Slippage} $:
  → Revert transaction
  → No trade executed
  → No fees charged
...
Space is researching zero-knowledge proofs for trustless subjective resolution:
